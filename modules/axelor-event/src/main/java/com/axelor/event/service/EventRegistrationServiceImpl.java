...
public class EventRegistrationServiceImpl implements EventRegistrationService {
  ...
  public EventRegistration compute(Event event, EventRegistration eventRegistration) {
    ...
    Integer diffCloseAndRegist = event.getRegistrationCloseDate().compareTo(eventRegistration.getRegistrationDateT().toLocalDate());
    System.err.println(diffCloseAndRegist); 
    List<Discount> discountList = event.getDiscountList();
    Discount discount = null ;
    if(discountList != null) {
      discount = discountList.stream().filter(a -> a.getBeforeDays()<=diffCloseAndRegist).max(Comparator.comparing(Discount::getBeforeDays)).orElse(null);
    }
    if(discount != null)
    {
